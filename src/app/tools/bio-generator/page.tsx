...
export default function BioGeneratorPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast({
        title: t("tools.bio.errorTitle"),
        description: t("tools.bio.errorGenerate"),
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const copyBio = (bio: string, index: number) => {
    ...
    toast({
      ...
      description: t("tools.bio.copiedDesc"),
    });
  ...
  return (
    <ToolLayout
      titleKey="tools.bio.title"
      descriptionKey="tools.bio.description"
        ...
        <Card className="shadow-2xl shadow-primary/10 border-0 bg-card/80 backdrop-blur-sm">
          <CardHeader className="text-center pb-2">
            <CardTitle className="text-2xl font-headline flex items-center justify-center gap-2">
              ...
              {t("tools.bio.formTitle")}
            </CardTitle>
            <CardDescription>{t("tools.bio.formDescription")}</CardDescription>
          </CardHeader>
          <CardContent className="pt-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Niche Select */}
              <div className="space-y-2">
                <Label>{t("tools.bio.nicheLabel")}</Label>
              ...
              {/* Vibe Select */}
              <div className="space-y-2">
                <Label>{t("tools.bio.vibeLabel")}</Label>
            ...
            {/* Keywords Input */}
            <div className="space-y-2">
              <Label>{t("tools.bio.keywordsLabel")}</Label>
              <Input
                value={keywords}
                onChange={(e) => setKeywords(e.target.value)}
                placeholder={t("tools.bio.keywordsPlaceholder")}
                className="h-12"
              />
              <p className="text-xs text-muted-foreground">{t("tools.bio.keywordsHint")}</p>
            </div>
            ...
            >
              {isLoading ? (
                <>
                  ...
                  {t("tools.bio.generating")}
                </>
              ) : (
                <>
                  <Sparkles className="mr-2 h-5 w-5" />
                  {t("tools.bio.generate")}
                </>
              )}
            </Button>
          </CardContent>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 space-y-3">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-sm text-muted-foreground">{t("tools.bio.suggestedEmojis")}:</span>
